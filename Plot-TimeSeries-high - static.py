...
for i, color in enumerate(colors, start=0):
    plt.plot(Timestamp, np.hsplit(Vhm, levels)[i+23], color=color, label='Vhm_{i}m'.format(i=(i+23) * 20 + 100))
...
ax2.grid(True)
for i, color in enumerate(colors, start=0):
    plt.plot(Timestamp, np.hsplit(CNRm, levels)[i+23], color=color, label='CNRm_{i}m'.format(i=(i+23) * 20 + 100))
...
plt.savefig('TimeSeries-high.png')
plt.show()